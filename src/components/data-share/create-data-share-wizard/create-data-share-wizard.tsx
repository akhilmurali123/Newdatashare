...
interface CreateDataShareWizardProps {
  isOpen: boolean;
  onClose: () => void;
...
function CreateDataShareWizardContent({ isOpen, onClose, onShareCreated, originPage, initialSelectedEntityTypes, initialIncludeRelationships, initialIncludeInteractions, initialDatasetName, initialDescription, initialTarget, hideSelectDataTypesStep }: CreateDataShareWizardProps) {
  ...
  const [dataConfig, setDataConfig] = useState<DataShareConfiguration>(() => {
    ...
    const preselectEntities = initialSelectedEntityTypes || searchParams.get('preselectEntities')?.split(',') || [];
    ...
    // Detect if opened from RIA for preview/review (ai-agent-flow with openWizard=true and step=reviewDataset or configureData)
    const isRiaPreview =
      (searchParams.get('openWizard') === 'true') &&
      ([ 'reviewDataset', 'configureData' ].includes(searchParams.get('step') || '')) &&
    ...
    if (isRiaPreview) {
      // Preselect all required types for RIA preview
    ...
    } else if (hideSelectDataTypesStep) {
      ...
      entityTypes = JSON.parse(JSON.stringify(initialEntityTypes)).map((et: EntityTypeConfig) => ({
        ...et,
        selected: preselectEntities.length > 0 ? preselectEntities.includes(et.id) : false
      }));
      relationshipTypes = JSON.parse(JSON.stringify(initialRelationshipTypes)).map((rt: ConfigurableDataCategory) => ({
        ...rt,
        selected: includeRelationships
      }));
      interactionTypes = JSON.parse(JSON.stringify(initialInteractionTypes)).map((it: ConfigurableDataCategory) => ({
        ...it,
        selected: includeInteractions
      }));
      otherDataTypes = JSON.parse(JSON.stringify(initialOtherDataTypesConfig)).map((ot: ConfigurableDataCategory) => ({
        ...ot,
        selected: false
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      } else if (initialStepFromUrl && WIZARD_PROGRESS_STEPS.some(s => s.id === initialStepFromUrl)) {
        setProgressStep(initialStepFromUrl as ConfigureProgressStep);
      }
    }
  }, [isOpen, hideSelectDataTypesStep, WIZARD_PROGRESS_STEPS]);
...
export default function CreateDataShareWizard(props: CreateDataShareWizardProps) {
  return (
    <Suspense fallback={<div>Loading...</div>}>
      <CreateDataShareWizardContent {...props} />
    </Suspense>
  );
}
